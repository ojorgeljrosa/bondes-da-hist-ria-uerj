---
name: frontend-dev
description: Use when building complete landing pages, marketing sites, or product pages — including AI-generated media assets (images, video, audio via MiniMax), persuasive copywriting (AIDA/PAS/FAB frameworks), cinematic scroll animations (Framer Motion/GSAP), and generative art (p5.js). For design aesthetics and UI composition without asset generation, use the frontend-design skill. For component libraries with shadcn/ui, use the shadcn skill. For Tailwind CSS configuration, use the tailwind-patterns skill.
license: MIT
metadata:
  version: "1.0.0"
  category: frontend
...
- Fonts: Poppins (headings), Source Sans 3 (body) — adjust per project requirements
